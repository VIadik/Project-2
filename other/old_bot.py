...
# from inline_keyboards import ikb_wifi
...
TOKEN = "5857418760:AAHzvHd_UWkLRPgAXBSLWF1eET_V3Kw0U90"
...
async def generate_qr(message: types.Message, state: FSMContext):
    ...
    await message.answer("Выберейте тип данных, который Вы хотите закодировать.")
...
async def get_name(message: Message, state: FSMContext):
    ...
    await message.answer("Выберете стандарт безопасности")
...
async def photo_handler(message: types.Message):
    if message.media_group_id not in used:
        ...
        await bot.send_document(message.from_user.id, file)
        subprocess.run(f"python3 clear_directory.py telegram-bot-api/bin/{TOKEN}/photos", shell=True)
